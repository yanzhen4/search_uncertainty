"""
自动化运行 behavior judge 脚本
处理 Debate_QA 和 Researchy_QA 文件夹下所有的 parsed_results.json 文件
"""
import json
import os
import pandas as pd
...
def generate_output_filename(input_path: str, script_dir: Path) -> str:
    """
    根据输入路径生成输出文件名
    格式: {QA类型}_{子文件夹名}_{other_prompts_}parsed_agent_outputs_with_behavior_judge.json
    
    Args:
        input_path: 输入的 parsed_results.json 文件路径
        script_dir: 脚本所在目录（Eval_llm）
    ...
    if qa_type is None or subfolder is None:
        # 如果无法解析，使用默认名称
        return str(script_dir / "parsed_agent_outputs_with_behavior_judge.json")
    
    # 构建文件名
    filename_parts = [qa_type, subfolder]
    if is_other_prompts:
        filename_parts.append("other_prompts")
    filename_parts.append("parsed_agent_outputs_with_behavior_judge.json")
...
def process_single_file(input_path: str, output_path: str = None, script_dir: Path = None):
    ...
    if output_path is None:
        # 自动生成输出路径：保存到 Eval_llm 目录，文件名包含 QA 类型、子文件夹和是否 other_prompts
