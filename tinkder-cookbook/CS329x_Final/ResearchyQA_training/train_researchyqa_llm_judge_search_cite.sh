...
echo "=========================================="
echo "🚀 ResearchyQA Training (LLM Judge - LARGE)"
echo "=========================================="
echo ""
...
QUESTIONS_FILE="CS329x_Final/ResearchyQA_training/ResearchyQA_questions_200.jsonl"

MODEL_SHORT=$(echo "$MODEL_NAME" | tr '[:upper:]' '[:lower:]' | sed 's/\//-/g')
JUDGE_SHORT=$(echo "$JUDGE_MODEL" | awk -F'/' '{print $NF}' | tr '[:upper:]' '[:lower:]')
TIMESTAMP=$(date +"%Y-%m-%d-%H-%M")
RUN_NAME="researchyqa_multiperspective_cite_${MODEL_SHORT}_bs${BATCH_SIZE}_gs${GROUP_SIZE}_judge-${JUDGE_SHORT}_thresh${QUALITY_THRESHOLD}_seed${SEED}_traj$((MAX_TRAJECTORY_TOKENS/1024))k_lr${LEARNING_RATE}_rank${LORA_RANK}_${TIMESTAMP}"
...
echo "📋 Configuration:"
echo "  Dataset: LARGE (200 questions - ALL used for training)"
...
if ! curl -s http://localhost:8000/api/v1/heartbeat > /dev/null 2>&1; then
    ...
    echo "Start: chroma run --host localhost --port 8000 --path CS329x_Final/ResearchyQA_training/chroma_db"
    exit 1
fi
...
echo "🚀 Starting training on LARGE dataset (200 questions - ALL for training)..."
echo ""
